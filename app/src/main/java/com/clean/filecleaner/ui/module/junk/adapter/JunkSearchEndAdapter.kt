...
class JunkSearchEndAdapter(
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return when (viewType) {
            0 -> ParentViewHolder(ItemJunkSearchEndParentBinding.inflate(LayoutInflater.from(parent.context), parent, false))
        ...
        when (holder) {
            is ParentViewHolder -> {
                ...
                if (JunkType.APP_CACHE == item.trashType && activity.isGrantAppCache().not()) {
                    ...
                    bind.loading.isEnabled = if (item.isLoading) false else if (JunkType.APP_CACHE == item.trashType) {
                        activity.isGrantAppCache() && item.subItems.isNotEmpty()
                    } else true
                    bind.root.setBackgroundResource(if (item.isOpen) R.drawable.shape_bg_white_top_8 else R.drawable.shape_bg_white_8)
                ...
                bind.size.text = if (item.trashType == JunkType.APP_CACHE) {
                    formatFileSize(activity, item.subItems.filterIsInstance<TrashItemCache>().sumOf { it.fileSize })
                } else {
            ...
            is ItemViewHolder -> {
                ...
                when (item) {
                    is TrashItemCache -> {
                        ...
                        val isNeedBottomCorner = (holder.layoutPosition == source.lastIndex) || (source.getOrNull(holder.layoutPosition + 1) is TrashItemParent)
                        bind.root.setBackgroundResource(if (isNeedBottomCorner) R.drawable.shape_bg_white_bottom_8 else R.drawable.shape_bg_white_0)
                ...
                bind.loading.setOnClickListener { _ ->
                    item.select = !item.select
                    runCatching {
                        val parentIndex = source.indexOfFirst { it.trashType == item.trashType }
                        if (-1 == parentIndex) return@setOnClickListener
                ...
                holder.itemView.setOnClickListener {
                    when (item) {
                        ...
                        is TrashItem -> {
                            CommonDialog(title = item.name, message = item.path, leftBtn = "", rightBtn = "OK", cancelable = true).show(
