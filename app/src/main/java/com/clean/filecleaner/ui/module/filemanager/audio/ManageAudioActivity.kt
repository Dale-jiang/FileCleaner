...
class ManageAudioActivity : BaseActivity<ActivityManageAudioBinding>() {
    ...
    companion object {
        val allAudioList = mutableListOf<FileInfo>()
    }
    ...
    private fun setUpAdapter() {
        with(binding) {
            adapter = ManageAudioAdapter(this@ManageAudioActivity, list = allAudioList,
                clickListener = {
    ...
    private suspend fun fetchAudioList() = withContext(Dispatchers.IO + SupervisorJob()) {
        try {
            ...
            withContext(Dispatchers.Main) {
                ...
                binding.num.text = "${allAudioList.size}"
                binding.loadingView.isVisible = false
                binding.bottomView.isVisible = allAudioList.isNotEmpty()
                binding.emptyView.isVisible = allAudioList.isEmpty()
                setUpAdapter()
                setCleanBtn()
            }
